...
description: "Install @mascotbot-sdk/lipsync-react and lipsync-core from the private npm registry, configure your .npmrc auth token, and pick the right development or production API key."
icon: "download"
...
```bash Vanilla / Node
pnpm add @mascotbot-sdk/lipsync-core
```
...
Pass configuration to `<LipsyncProvider>` or `MascotLipsyncClient.init`:
...
export default function Layout({ children }: { children: React.ReactNode }) {
  return <LipsyncProvider apiKey={process.env.NEXT_PUBLIC_MASCOT_KEY!}>{children}</LipsyncProvider>;
}
```
