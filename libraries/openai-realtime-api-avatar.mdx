...
- **WebRTC (recommended, cleanest).** The session plays the assistant audio into an `<audio>` element you supply; you `captureStream()` it. No extra SDK audio piece.
...
WebRTC self-plays (tap via `captureStream()`); WebSocket hands you PCM (play + tap via `createPCMStreamPlayer`). Both end at one `useLipsyncStream` call.
...
export function OpenAIAvatar() {
  ...
  const connect = async () => {
    ...
    setStream(audioEl.captureStream());
...
Confirm `status === "ready"`, the tap stream is set (WebRTC: `audioEl.captureStream()`; WebSocket: `player.outputStream`), and the Rive file uses artboard `Character` + state machine `mascotStateMachine` with inputs `100`–`118`.
