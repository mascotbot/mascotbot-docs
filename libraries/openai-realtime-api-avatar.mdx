...
- **WebRTC (recommended, cleanest).** The session plays the assistant audio into an `<audio>` element you supply; you tap it with the cross-browser [`tapElement`](/realtime/overview#tap-a-playing-element) helper. No extra SDK audio piece.
...
WebRTC self-plays (tap via the [`tapElement`](/realtime/overview#tap-a-playing-element) Web-Audio helper); WebSocket hands you PCM (play + tap via `createPCMStreamPlayer`). Both end at one `useLipsyncStream` call.
...
export function OpenAIAvatar() {
  ...
  const connect = async () => {
    ...
    // tapElement: cross-browser Web-Audio tap (Safari has no
    // captureStream) — see /realtime/overview#tap-a-playing-element
    setStream(tapElement(audioEl));
    teardownRef.current = () => void session.close();
  };
...
Confirm `status === "ready"`, the tap stream is set (WebRTC: `tapElement(audioEl)`; WebSocket: `player.outputStream`), and the Rive file uses artboard `Character` + state machine `mascotStateMachine` with inputs `100`–`118`.
