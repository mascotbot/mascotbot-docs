...
```mermaid
flowchart TD
    subgraph SERVER["Your Server / API Route"]
        direction TB
        SECRETS["API Key + System Instructions + Voice Config + Generation Config"]
        SECRETS --> CREATE["Google AI SDK: ai.authTokens.create"]
        CREATE --> GTOKEN["Ephemeral Token - single-use, config locked"]
        GTOKEN --> PROXY["Mascot Bot Proxy: POST /v1/get-signed-url"]
        PROXY --> WRAPPED["Wrapped Token"]
    end

    WRAPPED -->|"Only token + model leave the server"| RECEIVES

    subgraph CLIENT["Client Browser"]
        direction TB
        RECEIVES["Receives: baseUrl, ephemeralToken, model"] --> SDK["Google AI SDK: new GoogleGenAI"]
        SDK --> CONNECT["ai.live.connect - model only, no config"]
    end

    CONNECT -->|"WebSocket via Mascot Bot Proxy"| GEMINI["Gemini Live API + Viseme Injection"]

    style SERVER fill:#1a2332,stroke:#3b82f6,stroke-width:2px,color:#e2e8f0
    style CLIENT fill:#1a2332,stroke:#f59e0b,stroke-width:2px,color:#e2e8f0
    style SECRETS fill:#3b1a1a,stroke:#ef4444,color:#fca5a5
    style CREATE fill:#1e3a5f,stroke:#60a5fa,color:#e2e8f0
    style GTOKEN fill:#1e4d3a,stroke:#34d399,color:#e2e8f0
    style PROXY fill:#1e3a5f,stroke:#60a5fa,color:#e2e8f0
    style WRAPPED fill:#1e4d3a,stroke:#34d399,color:#e2e8f0
    style RECEIVES fill:#3b2e1a,stroke:#f59e0b,color:#e2e8f0
    style SDK fill:#3b2e1a,stroke:#f59e0b,color:#e2e8f0
    style CONNECT fill:#3b2e1a,stroke:#f59e0b,color:#e2e8f0
    style GEMINI fill:#1e3a5f,stroke:#8b5cf6,color:#e2e8f0
```
...
<Columns cols={2}>
  <Card title="Try the Live Demo" icon="play" href="https://mascot.bot/gemini-liveapi">
