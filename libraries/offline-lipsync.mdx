...
```tsx
"use client";
import { useLipsync, useProcessAudio, parseTimeline } from "@mascotbot-sdk/lipsync-react";
...
const client = await MascotLipsyncClient.init({ apiKey: "mascot_pub_…" });
...
```ts
import { framesToTimeline, timelineToCues } from "@mascotbot-sdk/lipsync-core";
...
```ts
import { parseTimeline, LipsyncError } from "@mascotbot-sdk/lipsync-core";
