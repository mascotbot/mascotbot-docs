...
<Info>
**Want to see a complete working example?** Check out our [open-source demo repository](https://github.com/mascotbot-templates/elevenlabs-avatar) with full implementation, or deploy it directly to Vercel with one click.
</Info>
        ...
        body: JSON.stringify({
          config: {
            ...
            provider_config: {
              ...
              // Optional: pass dynamic variables if provided
...
### Step 4: Using Dynamic Variables
...
Update your API route to accept dynamic variables:
...
#### Using Dynamic Variables in Your Component

Pass dynamic variables when starting a conversation:
...
export function ElevenLabsAvatar() {
  ...
  useMascotElevenlabs({ conversation });
  ...
  const dynamicVariables = {
    ...
    role: "developer",
    company: "Acme Corp",
    // Add any variables your agent uses
  };
  ...
  const startConversation = async () => {
    ...
    // Pass dynamic variables to the conversation
...
#### Complete Example with Dynamic Variables

Here's a full implementation showing personalized greetings:
...
export function PersonalizedAvatar({ userName, userRole }: AvatarProps) {
  const [isConnecting, setIsConnecting] = useState(false);
  ...
  return (
    <div>
      <MascotClient src="/mascot.riv" artboard="Character">
        <MascotRive />
      </MascotClient>
      
      <button onClick={startPersonalizedConversation}>
...
<Info>
Configure your ElevenLabs agent to use these variables in prompts like: "Good `{{time_of_day}}`, `{{name}}`! As a `{{role}}`, how can I help you today?"
</Info>

#### Important Notes

- **No SDK Conflicts**: Dynamic variables are handled entirely by ElevenLabs - Mascot Bot SDK simply passes them through
- **Type Support**: Variables can be strings, numbers, or booleans
- **Agent Configuration**: Remember to configure your ElevenLabs agent to use the dynamic variables
- **Testing**: Use hardcoded values during development, then switch to dynamic values in production
...
If your ElevenLabs agent isn't using the dynamic variables:

<Warning>
1. Ensure your agent configuration includes the variable placeholders (e.g., `{{name}}`)
2. Check that variables are passed both to the proxy endpoint AND to `startSession()`
3. Verify the variable names match exactly (case-sensitive)
4. Check browser console for any errors in the API route
</Warning>

```tsx
// Correct implementation
const dynamicVariables = { name: "John", role: "admin" };

// 1. Pass to proxy endpoint
const response = await fetch('/api/get-signed-url', {
  method: 'POST',
  body: JSON.stringify({ dynamicVariables })
});

// 2. Pass to conversation
await conversation.startSession({ 
  signedUrl,
  dynamicVariables  // Don't forget this!
});
```
