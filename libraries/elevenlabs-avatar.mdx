...
export function ElevenLabsAvatar() {
  ...
  const start = async () => {
    ...
    // Stash the <audio> ElevenLabs creates internally so we can captureStream() it.
    ...
    const iv = window.setInterval(() => {
      const el = w.__el as (HTMLAudioElement & { captureStream?: () => MediaStream }) | undefined;
      const s = el?.captureStream?.();
      if (s) { setStream(s); window.clearInterval(iv); }
...
<Warning>
Confirm `status === "ready"` from `useLipsync()`, that the captured `MediaStream` is non-null (the `window.Audio` patch must be installed **before** `Conversation.startSession`), and that the Rive file uses artboard `Character` + state machine `mascotStateMachine` with inputs `100`–`118`.
</Warning>
...
The audio is tapped at its playback point via `captureStream()`, so visemes are derived from exactly what the user hears — the mouth cannot run ahead of the voice.
