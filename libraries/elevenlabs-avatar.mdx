...
<Columns cols={3}>
  ...
  <Card title="Live Demo" icon="play" href="https://mascot.bot/elevenlabs-demo">
...
Declare `gesture` on the parent `<Mascot inputs={["gesture", ...]}>`
...
| Surface | Role |
|---|---|
| `<MascotProvider apiKey>` | Initializes the licensed avatar client. [Config](/installation#4-configure-the-client). |
...
<Warning>
Confirm `status === "ready"` from `useMascot()`, that the tapped `MediaStream` is non-null (the `window.Audio` patch must be installed **before** `Conversation.startSession`, and `el.srcObject` must be a `MediaStream`), and that the Rive file uses artboard `Character` + state machine `mascotStateMachine` with inputs `100`–`118`.
</Warning>
...
<Columns cols={2}>
  <Card title="Try Voice Avatar Demo" icon="play" href="https://mascot.bot/elevenlabs-demo">
