...
After `<LipsyncProvider>` (or `MascotLipsyncClient.init()`) the SDK runs an
...
`useLipsync()` exposes one value, `status`:
...
import { useLipsync, RefusedError } from "@mascotbot-sdk/lipsync-react";

function SpeakButton() {
  const { client, status, reload } = useLipsync();
...
`reload()` tears the client down and runs `MascotLipsyncClient.init()` again —
...
```ts
import { MascotLipsyncClient, RefusedError } from "@mascotbot-sdk/lipsync-core";

let client = await MascotLipsyncClient.init({ apiKey: "mascot_pub_…" });

function wire(c: MascotLipsyncClient) {
  c.on("refused", async (_err) => {
    await c.close();
    client = await MascotLipsyncClient.init({ apiKey: "mascot_pub_…" });
    wire(client);
  });
}
wire(client);
...
Reading `useLipsync().error` gives you the typed cause when `status` is
...
<Columns cols={3}>
  ...
  <Card title="React hooks" icon="react" href="/libraries/react-hooks">
    `useLipsync` — the `status` / `error` / `reload` surface.
  </Card>
