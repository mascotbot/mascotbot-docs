...
```bash
pnpm install mascotbot-sdk-react-0.1.0.tgz
```
...
To use the SDK, wrap your application with the `MascotProvider` component. This component requires an API key to function.
...
function App() {
  return (
    <MascotProvider apiKey="your_api_key">
...
The `useMascot` hook provides access to the Rive component and its loading state.

```jsx
import { useMascot } from "@mascotbot-sdk/react";

const { RiveComponent, isRiveLoaded } = useMascot();
```

### `useMascotPlayback`

The `useMascotPlayback` hook allows you to control the playback of animations and manage visemes and stress levels.
...
To create a synchronized animation and audio experience, you can use the `playback.add` method to add visemes and audio. This can be done in various ways, such as fetching from a server, generating in real-time, or any other method suitable for your application.
