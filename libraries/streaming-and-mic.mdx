...
function play() {
  const el = audioRef.current as HTMLAudioElement & { captureStream?: () => MediaStream };
  setStream(el.captureStream?.() ?? null);
  el.play();
}
...
This is the same tap used for self-playing realtime providers (ElevenLabs,
OpenAI Realtime over WebRTC) — see [Realtime providers](/realtime/overview).
