...
```tsx
import { createElementTap, type ElementTap } from "@mascotbot-sdk/lipsync-react";
...
`createElementTap()` is an SDK export (`@mascotbot-sdk/lipsync-react`,
...
component. On **SDK 0.2.6+** the Rive input handles are referentially
stable and playback is carried across any internal recreate, so an
unrelated parent re-render no longer freezes the mouth — keeping the
`<MascotClient>` subtree from re-rendering is now optional render hygiene,
