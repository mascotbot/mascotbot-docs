...
Every mascot above ships as a Rive file with the artboard `Character` and the
`mascotStateMachine` state machine the SDK expects — drop one into
`<MascotClient src>` and it lip-syncs with no extra setup:
...
The SDK only animates the mouth, `is_speaking`, and `stress` — every other
input on these files (gestures, expressions, scene state) stays yours to
drive directly. New here? Start with the
[Quickstart](/quickstart), then the [React SDK](/libraries/react-sdk).